package net.gy.SwiftFrameWork.UI.customwidget.progresswheel;
...
public class ProgressWheel extends View {
  ...
  private void setupBounds(int layout_width, int layout_height) {
    ...
    if (!fillRadius) {
      // Width should equal to Height, find the min type to setup the circle
  ...
  /**
   * Set the progress to a specific type,
   * the bar will be set instantly to that type
   *
  ...
  /**
   * Set the progress to a specific type,
   * the bar will smoothly animate until that type
   *
   * @param progress the progress between 0 and 1
   */
   ...
   * (1.0 equals on full turn in one second), this type also is applied for
  ...
  public interface ProgressCallback {
    /**
     * Method to call when the progress reaches a type
     * in order to avoid float precision issues, the progress
     * is rounded to a float with two decimals.
     *
     * In indeterminate mode, the callback is called each time
     * the wheel completes an animation cycle, with, the progress type is -1.0f
     *
     * @param progress a double type between 0.00 and 1.00 both included
     */
